...
public class Proba extends Decisiveness{
    ...
    public final int UN_PAR_DEUX = 1/2;
    ...
    /**
     * Builded the ia whith the save
     * @param player copy the player
     * @throws IOException 
     */

    public Proba(final Player player) throws IOException {
        super();
        this.player = player;
        etatPrecedent = Etat.FreeBox;
    }
    ...
    public boolean move(final Map map, final String direction) {
        ...
        if(!carryOnNorth()) {
            probaSouth = probaEast = probaWest = UN_PAR_QUATRE;
